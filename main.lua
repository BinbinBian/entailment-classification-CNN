...
cmd:text()
cmd:text('Options')
cmd:option('-model_type', 'static', 'Model type. Options: rand (randomly initialized word embeddings), static (pre-trained embeddings from word2vec, static during learning), nonstatic (pre-trained embeddings, tuned during learning), multichannel (two embedding channels, one static and one nonstatic)')
cmd:option('-data', 'snli-w2v.hdf5', 'Training data and word2vec data')
cmd:option('-cudnn', 1, 'Use cudnn and GPUs if set to 1, otherwise set to 0')
cmd:option('-seed', 3216, 'random seed, set -1 for actual random')
...
cmd:option('-num_classes', 3, 'Number of output classes')
  ...
  for fold = 1, opt.folds do
    ...
    for epoch = 1, opt.num_epochs do
      ...
      if opt.savefile ~= '' then
        ...
        savefile = string.format('results/temp_model.t7')
      end
      print('saving preliminary epoch results to ', savefile)
